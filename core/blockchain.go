...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool) (status WriteStatus, err error) {
	...
	log.Debug("State commit", "time", common.PrettyDuration(time.Since(commitStart)))
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		log.Debug("Triedb commit", "time", common.PrettyDuration(time.Since(trieCommitStart)))
	...
	// thunder_patch begin
	// Pala doesn't update canonical until reach some kind of consensus.
	if bc.Config().Thunder.IsPala(block.Number()) {
		status = SideStatTy
	} else {
		// thunder_patch end
...
func (bc *BlockChain) WriteKnownBlock(block *types.Block) error {
	...
	if receipts == nil {
		msg := fmt.Sprintf("Cannot get receipts for blockhash: %s", block.Hash().Hex())
		log.Error(msg)
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool) (int, error) {
	...
	// thunder_patch begin
	currentBlock := bc.CurrentBlock()
	session := bc.Config().Thunder.GetSessionFromDifficulty(currentBlock.Difficulty(), currentBlock.Number(), bc.Config().Thunder)
	senderCacher.recoverFromBlocks(types.MakeSigner(bc.chainConfig, chain[0].Number(), session), chain)
