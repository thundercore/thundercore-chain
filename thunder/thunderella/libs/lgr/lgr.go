...
func (lgr *Lgr) Note(s string, args ...interface{}) {
	lgr.output(LvlNote, s, args)
}
...
var verboseLoggerLevel = LvlInfo
...
func (lgr *Lgr) output(level int64, format string, args []interface{}) {
	...
	loggerShouldOutput := lgr.shouldOutput(level)
	verboseLoggerShouldOutput := verboseLogger != nil && level >= verboseLoggerLevel
	if !loggerShouldOutput && !verboseLoggerShouldOutput {
		return
	}
	...
	if loggerShouldOutput {
		logger.Output(0, outputString) // calldepth not used
	}
	if verboseLoggerShouldOutput {
		verboseLogger.Output(0, outputString) // calldepth not used
	}
}
