...
// SetVerboseLogFileLevel sets the level of the "more verbose" log file that's
// typically kept for shorter duration and not uploaded
func SetVerboseLogFileLevel(level int64) error {
	if level < 0 || level >= LvlCount {
		return fmt.Errorf("SetVerboseLogFileLevel: bad level %d", level)
	}
	verboseLoggerLevel = level
	return nil
}
