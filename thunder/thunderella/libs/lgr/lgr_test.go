...
func TestOutputHandling(t *testing.T) {
	if utils.InBenchmark() {
		t.Skip("Log output suppressed in benchmark mode.")
	}
	clearLogDomains()
	assert := assert.New(t)
	...
	baseLogger.Debug("debug1")
	baseLogger.Info("info1")
	baseLogger.Note("note1")
	...
	// set "/A/B" to level "note", it logs stuff and make sure "/A" does not
	SetLogLevel("/A/B", LvlNote)
	abLogger.Note("note2")
	assert.True(writer.Len() > currentLength, "didn't log something to /A/B that it should have")
	currentLength = writer.Len()
	aLogger.Note("note3")
