...
func TestOrderG1(t *testing.T) {
	...
	// Test the "e.p == nil" case of G1.Add
	a := new(G1).ScalarBaseMult(new(big.Int).SetInt64(1))
	b := new(G1).ScalarBaseMult(new(big.Int))
	g = &G1{}
	g.Add(b, a)
	g.p.MakeAffine(nil)
	if g.p.x.Cmp(a.p.x) != 0 || g.p.y.Cmp(a.p.y) != 0 {
		t.Errorf("1+0 != 1 in G1")
	}
}
...
func TestOrderG2(t *testing.T) {
	...
	// Test the "e.p == nil" case of G2.Add
	a := new(G2).ScalarBaseMult(new(big.Int).SetInt64(1))
	b := new(G2).ScalarBaseMult(new(big.Int))
	g = &G2{}
	g.Add(b, a)
	g.p.MakeAffine(nil)
	if g.p.x.x.Cmp(one.p.x.x) != 0 ||
...
func TestG2Random(t *testing.T) {
	_, g2, _ := RandomG2(rand.Reader)
	g2n, _ := new(G2).Unmarshal(g2.Marshal())
	if !reflect.DeepEqual(g2, g2n) {
		t.Fatalf("random marshal failed")
	}
}
